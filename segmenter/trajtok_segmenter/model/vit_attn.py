...
class ViTSelfAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None:
            attention_scores = attention_scores.masked_fill(attention_mask.unsqueeze(1).unsqueeze(2) == 0, float('-inf'))
...
class ViTLayer(nn.Module):
    ...
    def __init__(self, config: ViTConfig) -> None:
        ...
        self.seq_len_dim = 1
        if config.use_x_former: 
            self.attention = ViTXSelfAttention(config)
        else:
