...
def torch_arange_like(length, ref, start=0):
    return torch.arange(start, start+length, device=ref.device, dtype=ref.dtype)
...
def load_temp_embed_with_mismatch(temp_embed_old, temp_embed_new, add_zero=True):
    ...
    if num_frms_new > num_frms_old:
        if add_zero:
            ...
            temp_embed_new = interpolate_temporal_pos_embed(num_frms_old, num_frms_new)
