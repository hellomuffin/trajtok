...
def setup_wandb(config, eval=False, manual_version=None, eval_name=None):
    if not (config.wandb.enable and is_main_process()):
        return
    project_name = 'open_trajvit'
    
    input_config = dict(deepcopy(config))
    input_config.pop("available_corpus")
    
    group = config.wandb.group
    name = os.path.basename(config.output_dir)
    if config.video_input.num_frames_test == 1: name += "_image_eval"
    if config.mask_down_factor!=4: name += f"mask_down_{config.mask_down_factor}"

    if config.video_input.version_ext != '_v0': name += f'data_version_{config.video_input.version_ext}'
