...
# Import stdlib `typing` first — cv2 4.x ships a `cv2.typing` submodule that,
# in DataLoader worker subprocesses spawned later, can shadow Python's stdlib
# `typing` and crash with `ImportError: cannot import name 'NamedTuple'`.
# Anchoring stdlib `typing` in sys.modules here prevents that race.
...
# Use the NEW SigLip2+segmenter-connector backbone (matches the trajvlm_pretrain_v2
# checkpoint we're loading from). The old TrajVitVisionBackboneConfig
# (DINOv3-as-backbone) was discarded in the v2 architecture pivot.
from olmo.nn.siglip2_trajgroup_vision_backbone import (
    SigLip2TrajGroupVisionBackboneConfig,
)
from olmo.preprocessing.trajvit_preprocessor import TrajVitImageConfig, TrajVitVideoConfig
...
def get_trajvlm_model(checkpoint, *, num_frames: int, frames_per_clip: int):
    ...
    # Image preprocessor: keep the trajvit-specific one from pretrain as-is.
    if not isinstance(model_cfg.mm_preprocessor.image, TrajVitImageConfig):
        raise NotImplementedError(
            f"expected mm_preprocessor.image to be TrajVitImageConfig, got "
            f"{type(model_cfg.mm_preprocessor.image).__name__}. The SigLip2 backbone "
            f"doesn't expose image_res/normalize_on_gpu directly (those live on vit), "
            f"so we can't safely reconstruct."
        )
    image_preproc = model_cfg.mm_preprocessor.image

    # Video preprocessor: pretrain used (num_frames=1, frames_per_clip=1) since
    # PixMoCap is image-only; SFT needs the full video spec. Take the pretrain
    # video preproc (already a TrajVitVideoConfig with the correct
    # image_res/num_traj/normalize_on_gpu copied from the backbone at pretrain
    # time) and override only num_frames + frames_per_clip.
    if not isinstance(model_cfg.mm_preprocessor.video, TrajVitVideoConfig):
        raise NotImplementedError(
            f"expected mm_preprocessor.video to be TrajVitVideoConfig, got "
            f"{type(model_cfg.mm_preprocessor.video).__name__}."
        )
    video_preproc = dataclasses.replace(
        model_cfg.mm_preprocessor.video,
...
def _start_memory_profiler(log_interval_sec: int = 30):
    """Daemon thread that prints per-rank RSS + /dev/shm usage + system free RAM.
    Helps diagnose host-RAM OOMs that don't surface in the trainer's own logs."""
    import os, sys, threading, time, shutil, socket
    try:
        import psutil
    except ImportError:
        print("[mem-profiler] psutil unavailable; skipping", file=sys.stderr, flush=True)
        return
    ...
    def loop():
        while True:
            try:
                ...
                # Sum RSS of children too (dataloader workers)
                child_rss_gb = sum(c.memory_info().rss for c in proc.children(recursive=True)) / 1024**3
                vm = psutil.virtual_memory()
                shm = shutil.disk_usage("/dev/shm")
                print(
                    f"[mem-profiler] host={host} rank={rank}/local={local_rank} "
                    f"main_rss={rss_gb:.2f}GB children_rss={child_rss_gb:.2f}GB "
                    f"sys_used={vm.used/1024**3:.1f}GB/{vm.total/1024**3:.1f}GB "
                    f"sys_free={vm.available/1024**3:.1f}GB "
                    f"shm_used={shm.used/1024**3:.1f}GB/{shm.total/1024**3:.1f}GB",
                    file=sys.stderr, flush=True,
                )
            except Exception as e:
                print(f"[mem-profiler] err: {e}", file=sys.stderr, flush=True)
            time.sleep(log_interval_sec)
    t = threading.Thread(target=loop, daemon=True, name="mem-profiler")
    t.start()


def main():
    prepare_torchrun_environment()
    # _start_memory_profiler(30)  # disabled — re-enable when investigating host-RAM OOMs
