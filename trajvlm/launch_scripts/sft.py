...
IMAGE_ACADEMIC_DATASETS = [
    ...
    # Multi-image — removed because TrajVitImagePreprocessor does not support
    # multi-image inputs (raises "This preprocessor does not support multi-image"
    # in olmo/preprocessing/multimodal_preprocessor.py). The paper architecture
    # is single-image / single-video per sample.
    ...
    # Synthetic cosyn multi-doc — REMOVED: PixmoMulitDocQa returns multi-image
    # samples (max_images=5), unsupported by TrajVitImagePreprocessor.
...
VIDEO_ACADEMIC_DATASETS = [
    ...
    # our QA — registry uses molmo2_* names (renamed from vixmo_syn_video_capqa_v2 / _with_sub_v2)
    "molmo2_syn_captions_qa",            # 200K general qa  (allenai/Molmo2-VideoCapQA)
    "molmo2_syn_captions_subtitle_qa",   # 100K subtitle qa (allenai/Molmo2-VideoSubtitleQA)
]
...
def get_training_mixture(name):
    if name == "debug":
        training_mixture = [
            ...
            ["nlp", ["tulu4"], 0.1],
            # ["hardcodes", ["molmo2_hardcodes"], 0.001],  # ~17% multi-image, unsupported by TrajVit
        ]
    elif name == "tracking":
        ...
        training_mixture = [
            ["demo", [
                ...
                # "pixmo_multi_image_qa_multi_only_max5",  # multi-image not supported by TrajVit
                "molmo2_human_qa",
            ...
            ["image_pointing", [
                # pixmo_multi_points removed: PixMoMultiPoints uses
                # multi_image_pointing / multi_image_point_then_count styles,
                # unsupported by TrajVitImagePreprocessor.
            ...
            ["nlp", ["tulu4"], 0.1],
            # ["hardcodes", ["molmo2_hardcodes"], hardcode_weight],  # ~17% multi-image, unsupported by TrajVit
        ]
