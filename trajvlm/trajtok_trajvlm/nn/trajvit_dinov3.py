...
# DINOv3 ConvNeXt weight files + repo root are located via env vars:
#   TRAJTOK_DINOV3_ROOT          : directory containing the cloned dinov3 repo
#                                  (must have hubconf.py). Default ./dinov3.
#   TRAJTOK_DINOV3_WEIGHTS_DIR   : directory holding dinov3_convnext_{size}_pretrain_*.pth
#                                  files. Defaults to TRAJTOK_DINOV3_ROOT.
# See trajvlm/README.md for one-line download instructions.
import glob as _glob


def _dinov3_root() -> str:
    return os.environ.get("TRAJTOK_DINOV3_ROOT", "./dinov3")


def _dinov3_weights_path(model_size: str) -> "str | None":
    weights_dir = os.environ.get("TRAJTOK_DINOV3_WEIGHTS_DIR", _dinov3_root())
...
class DINOResNetHierFeat(nn.Module):
    def __init__(
        ...
        if load_pretrained:
            ckpt_path = _dinov3_weights_path(model_size)
            if ckpt_path is None:
                raise FileNotFoundError(
                    f"DINOv3 ConvNeXt-{model_size} weights not found. Set "
                    "TRAJTOK_DINOV3_WEIGHTS_DIR to the directory containing "
                    f"dinov3_convnext_{model_size}_pretrain_*.pth (see README.md)."
                )
            self.backbone = torch.hub.load(
                _dinov3_root(), f"dinov3_convnext_{model_size}",
...
class SigLipFeat(nn.Module):
    def __init__(
        ...
        super().__init__()
        # SigLipFeat is an optional ablation backbone — requires molmo2 source
        # on PYTHONPATH for its `VisionTransformer`. Skip if you're only using
        # the DINOv3 backbone (DINOResNetHierFeat above).
        try:
            from olmo.nn.image_vit import VitConfig, VisionTransformer
        except ImportError as e:
            raise ImportError(
                "SigLipFeat requires the molmo2 package on PYTHONPATH for "
                "olmo.nn.image_vit.VisionTransformer."
            ) from e
