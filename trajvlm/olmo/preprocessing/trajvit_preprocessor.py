...
class TrajVitVideoPreprocessor:
    ...
    def load_video(self, *args, **kwargs):
        # Mirror VideoPreprocessor.load_video — delegate to the shared
        # VideoLoader (built via TrajVitVideoConfig.build_video_loader). The
        # molmo2 multimodal preprocessor calls this on every video example.
        if self.video_loader is None:
            raise NotImplementedError(
                "TrajVitVideoPreprocessor has no video_loader. Build via "
                "TrajVitVideoConfig.build_video_preprocessor() so the loader is wired."
            )
